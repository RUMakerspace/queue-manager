...
def extractPrusaThumbnails(data):
	...
	matches = re.findall(r'(thumbnail begin.*?\n)(.*?)(; thumbnail end)', data, flags=re.S)
	
	thumbs = []
	
	for m in matches: #Remove the line endings, and remove the newlines.  Should work then.
...
def extractPrusaGCodeInfo(filename, fileData):
	...
	importantKeys = ["estimated printing time",
